...
const SidebarRoot = React.forwardRef<HTMLDivElement, SidebarProps>(
  (
    ...
    const { state, setSidebarState, toggleSidebar, isMobile } = useSidebar();
    ...
    // Handle Backdrop Click (for overlay mode without hover)
      ...
      return (
        <SidebarContext.Provider value={contextValue}>
          <div className="relative h-full flex shrink-0">
            {/* 
               Ghost Placeholder: 
               Keeps the layout from shifting by reserving space equal to the collapsed width.
            */}
            <motion.div
              initial={false}
            ...
            {/* 
               Backdrop / Mask:
               Darkens the background content when expanded.
            */}
            <AnimatePresence>
              {!isCollapsed && (
                <motion.div
...
const sidebarItemVariants = cva(
  "group relative flex w-full items-center border border-transparent font-medium outline-none transition-colors duration-200 hover:bg-graphite-secondary/60 focus-visible:ring-2 focus-visible:ring-graphite-ring",
  {
    variants: {
      isActive: {
        true: "bg-graphite-secondary text-graphite-foreground font-semibold",
        false: "text-graphite-foreground/70 hover:text-graphite-foreground",
      },
      size: {
        sm: "py-1.5 text-xs",
        md: "py-2 text-sm",
...
const SidebarItem = React.forwardRef<HTMLButtonElement, SidebarItemProps>(
  ({ className, icon, children, isActive, badge, ...props }, ref) => {
    const { state, size: contextSize, shape: contextShape } = useSidebar();
    const isCollapsed = state === "collapsed";
    const localRef = useRef<HTMLButtonElement>(null);
    React.useImperativeHandle(ref, () => localRef.current!);
...
const SidebarLabel = ({
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: state === "collapsed" ? 0 : 1, y: 0 }}
