...
const tableContainerVariants = cva("w-full overflow-hidden", {
  variants: {
    variant: {
      primary:
...
const tdVariants = cva("p-4 align-middle [&:has([role=checkbox])]:pr-0", {
  ...
  // Removed compound variants that forced a specific background color
});
...
>(({ className, colIndex, ...props }, ref) => {
  const { responsiveLayout, density, variant } = useTableContext();
  ...
  return (
    <th
      ref={ref}
      className={clsx(
        thVariants({ density, variant, isFirstSticky }),
        // Manually apply background for sticky header to prevent transparency issues
        isFirstSticky && variant === "primary" && "bg-graphite-secondary/50",
        isFirstSticky && variant === "secondary" && "bg-graphite-background",
        className
      )}
      {...props}
    />
...
>(({ className, colIndex, ...props }, ref) => {
  ...
  return (
    <td
      ref={ref}
      className={clsx(
        tdVariants({ density, variant, isFirstSticky }),
        // Ensure sticky columns have a background matching the row context to hide scrolled content
        // We use 'inherit' or the card background usually, but to "look normal" (match the row hover),
        // we mostly rely on the TR background. However, to prevent see-through on scroll:
        isFirstSticky && "bg-inherit",
        className
      )}
      {...props}
    />
...
const TableRoot = <TData extends {}>({
  ...
  const contextValue = useMemo(
    ...
    [table, responsiveLayout, isMobile, density, variant, renderContextMenu]
  );
