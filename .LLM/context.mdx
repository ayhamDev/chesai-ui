...
// This is shown when the user is pulling down.
const DefaultRefreshIndicator = ({ y }: { y: any }) => {
  // Rotate the arrow as the user pulls
  const rotate = useTransform(y, [0, PULL_THRESHOLD], [0, 180]);
  return (
    <motion.div
      style={{ rotate }}
      className="flex h-10 w-10 items-center justify-center rounded-full bg-graphite-secondary text-graphite-secondaryForeground"
    >
...
export interface PullToRefreshProps {
  ...
  /** A custom component or element to display while refreshing. Defaults to the pull indicator. */
...
export const PullToRefresh = ({
  ...
  // Determine if we're on a touch-enabled device
  const isTouchDevice =
    typeof window !== "undefined" &&
    ("ontouchstart" in window || navigator.maxTouchPoints > 0);

  // Animation to show while actively refreshing
  const scale = useTransform(
    y,
    [0, PULL_THRESHOLD],
    [0, 1],
    { clamp: true } // Don't let the indicator scale beyond 1
  );

  const handleRefresh = useCallback(async () => {
    setIsRefreshing(true);
    await onRefresh();
    // Use a finally block to ensure state is reset even if onRefresh fails
  ...
  const handleDragEnd = async (
    ...
    if (info.offset.y >= PULL_THRESHOLD) {
      // User pulled far enough, trigger the refresh
      // 1. Animate the indicator to its "loading" position
      animate(y, LOADING_INDICATOR_HEIGHT, { type: "spring", damping: 30 });
      // 2. Call the refresh logic
      await handleRefresh();
      // 3. Once refresh is done, animate back to the start
      animate(y, 0, { type: "spring", damping: 30 });
    } else {
      // User didn't pull far enough, snap back to the start
      animate(y, 0, { type: "spring", damping: 30 });
    }
  };
  ...
  return (
    <div className={clsx("relative w-full overflow-hidden", className)}>
      {/* 1. The Indicator Area */}
      <motion.div
        className="absolute inset-x-0 top-0 z-0 flex justify-center pt-4"
        style={{ scale }}
      >
        {isRefreshing ? (
          refreshingIndicator ?? (
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-graphite-secondary text-graphite-secondaryForeground">
      ...
      {/* 2. The Draggable Content Area */}
      <motion.div
        className={clsx("relative z-10 bg-inherit", contentClassName)}
        style={{ y }}
        // Only enable dragging on touch devices
        drag={"y"}
        // Constrain the drag to the vertical axis and give it a rubber-band effect
        dragConstraints={{ top: 0, bottom: PULL_THRESHOLD * 1.5 }}
        dragElastic={0.2}
        onDragEnd={handleDragEnd}
        // Animate presence for smooth appearance
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 0.2 }}
      >
