...
const navItemVariants = cva(
  "relative flex items-center w-full h-14 px-5 rounded-full transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-graphite-ring focus-visible:ring-offset-2",
  {
    variants: {
      isActive: {
...
const SidebarRoot: React.FC<SidebarProps> = ({
  ...
  const contextValue = useMemo(
    () => ({
      isDesktop,
      isCollapsed: isDesktop ? isCollapsed : false, // Collapse is a desktop-only feature
      isOpenOnMobile,
      activeItem,
      desktopVariant,
      mobileVariant,
        ...
        {isDesktop ? (
          ...
          <>
            <AnimatePresence>
              {isOpenOnMobile && (
                <>
                  ...
                  {React.cloneElement(sidebarContainer, {
                    // Inject mobile-specific props
                    _mobile: true,
                  })}
                </>
              )}
            ...
            <motion.div
              ...
              animate={{
                x:
                  isOpenOnMobile && mobileVariant === "push"
                    ? side === "left"
                      ? expandedWidth
                      : -expandedWidth
                    : 0,
...
interface SidebarContainerProps {
  children: React.ReactNode;
  className?: string;
  _mobile?: boolean; // Internal prop
}
const SidebarContainer = React.forwardRef<HTMLElement, SidebarContainerProps>(
  ({ children, className, _mobile = false }, ref) => {
    const { side, isDesktop } = useSidebar();
    const x = useMotionValue(0);

    const handleDragEnd = (e: MouseEvent | TouchEvent, info: PanInfo) => {
      const { setIsOpenOnMobile } = useSidebar.getState(); // Access latest state
      const dragThreshold = 50;
      const isClosing =
        (side === "left" && info.offset.x < -dragThreshold) ||
        (side === "right" && info.offset.x > dragThreshold);
    ...
    return (
      <motion.aside
        ...
        onDragEnd={handleDragEnd}
        dragConstraints={{
          left: side === "left" ? 0 : -Infinity,
          right: side === "right" ? 0 : Infinity,
        }}
        ...
        transition={{ type: "spring", stiffness: 500, damping: 50 }}
        className={clsx(
          "fixed top-0 bottom-0 z-50 flex h-full w-[280px] flex-col bg-graphite-secondary shadow-lg",
...
const SidebarPrimaryAction = React.forwardRef<
  HTMLButtonElement,
  PrimaryActionProps
  ...
  return (
    <div className="px-3 py-2">
      <motion.button
        ...
        layout
        transition={{ type: "spring", stiffness: 400, damping: 40 }}
        className={primaryActionVariants({ variant, isCollapsed, className })}
        {...props}
      >
...
// --- SidebarNav ---
const SidebarNav = (props: React.HTMLAttributes<HTMLElement>) => (
  <nav
    className={clsx("flex-1 space-y-1 px-3 py-2", props.className)}
    {...props}
  />
);
...
const SidebarItem = React.forwardRef<HTMLButtonElement, NavItemProps>(
  ({ itemKey, icon, children, endAdornment, className, ...props }, ref) => {
    ...
    return (
      <button
        ...
        className={navItemVariants({ isActive, isCollapsed, className })}
        {...props}
      >
        {isActive && (
          <motion.div
...
// --- SidebarSeparator ---
const SidebarSeparator = (props: React.HTMLAttributes<HTMLHRElement>) => (
  <hr
...
const SidebarSectionHeader = (props: React.HTMLAttributes<HTMLDivElement>) => {
  const { isCollapsed } = useSidebar();
  return (
    <AnimatePresence>
      {!isCollapsed && (
...
>((props, ref) => {
  ...
  return (
    <IconButton ref={ref} onClick={() => setIsOpenOnMobile(true)} {...props} />
  );
});
