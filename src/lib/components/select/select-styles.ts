...
export const selectContentVariants = cva('z-50 overflow-hidden bg-surface-container relative', {
  variants: {
    position: {
      popper: [
      ...
      'item-aligned': [
        'shadow-xl border border-outline-variant',
    ...
    variant: {
      flat: 'bg-surface-container-highest',
      faded: 'bg-surface-container/50 backdrop-blur-md',
      bordered: 'bg-surface-container',
      underlined: 'bg-surface-container',
    },
  },
  defaultVariants: {
    position: 'popper',
...
export const selectSlots = {
  ...
  // Removed static 'items-end' from here; it is now dynamic
...
export const getSelectSlotClassNames = (
  props: VariantProps<typeof selectStyles> & { isFilled?: boolean; hasStartContent?: boolean },
) => {
  const { variant, size, labelPlacement, isInvalid, shape, hasStartContent } = props
  ...
  let innerWrapperAlign = 'items-center' // Default to center for outside labels

  if (labelPlacement === 'inside') {
    innerWrapperAlign = 'items-end' // Push text down to make room for label
