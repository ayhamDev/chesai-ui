...
export const getInputSlotClassNames = (
  props: VariantProps<typeof inputStyles> & { isFilled?: boolean; hasStartContent?: boolean },
) => {
...
/**
 * Shared CVA for input wrappers (used by Select, TimePicker, etc)
 */
export const inputWrapperVariants = cva(
  'relative w-full inline-flex tap-highlight-transparent flex-row items-center gap-3 transition-colors duration-200 ease-out overflow-hidden outline-none text-left',
  {
    ...
    compoundVariants: [
      ...
      // Shape + Underlined overrides (removes side padding/rounding for underlined)
