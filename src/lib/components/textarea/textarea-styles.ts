...
export const getTextareaSlotClassNames = (props: VariantProps<typeof textareaStyles> & { isFilled?: boolean }) => {
  ...
  if (shape === "full") rounding = "rounded-3xl"; // Slightly larger for textareas
  ...
  // --- 4. SIZING & PADDING ---
  // Textareas use py for vertical padding
  let py = "py-2";
  let px = "px-3";
  ...
  if (labelPlacement === "inside") {
    // Start position: top-4 (standard padding)
    labelClasses += " absolute top-3 font-normal";
    
    // Filled State (Float to top)
    ...
    // Push input text down so it doesn't overlap the floating label
    if (props.isFilled) {
